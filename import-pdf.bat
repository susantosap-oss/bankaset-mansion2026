...
if not "%~1"=="" (
  ...
  echo Drag file PDF ke atas icon import-pdf.bat,
  echo atau ketik path di bawah ini:
  echo.
  set /p PDF_PATH="Path PDF : "
)
if "%PDF_PATH%"=="" ( echo Path tidak boleh kosong. & pause & exit /b 1 )
if not exist "%PDF_PATH%" ( echo File tidak ditemukan: %PDF_PATH% & pause & exit /b 1 )

echo.

:: ── Nama Bank ─────────────────────────────────────────
set /p BANK_NAME="Nama Bank : "
if "%BANK_NAME%"=="" ( echo Nama bank tidak boleh kosong. & pause & exit /b 1 )
...
echo   (proses berjalan, jangan tutup window ini)
echo =====================================================
echo.
